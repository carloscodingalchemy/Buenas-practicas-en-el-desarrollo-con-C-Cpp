...
int32_t t_fine;
int32_t compensate_temperature(int32_t adc_T);int32_t compensate_pressure(int32_t adc_P);
...
typedef struct {
    ...
    int16_t dig_P8;
    int16_t dig_P9;
} BMP280_CalibData;uint8_t bmp280_read(uint8_t reg){
    ...
    uint8_t buf[TEMP_PRESS_CALIB_DATA_LEN];for(int i = 0; i < TEMP_PRESS_CALIB_DATA_LEN; i++){
...
}int32_t compensate_temperature(int32_t adc_T,BMP280_CalibData* calib){int32_t var1,var2,T;
    ...
    return T;
}int32_t compensate_pressure(int32_t adc_P,BMP280_CalibData* calib){int64_t var1,var2,p;
